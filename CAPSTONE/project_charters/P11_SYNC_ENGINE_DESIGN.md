# P11 Sync Engine Design

**Status:** Implemented. Core: `memory/sync/`, `routers/sync.py`; push/pull/trigger; LWW merge; selective sync by space.  
**Goals:** CRDT-based sync, offline-first, selective sync per space.  
**Reference:** Current stack: Qdrant + Neo4j, Spaces, user_id.
...
- Mnemo charter and sync goal (Phase 4).  
...
These items come from the charter but are out of scope for Phase 4 v1:
...
- **Apply latency:** Target ≤100 ms for applying pulled changes so synced memories are searchable promptly; measure and tune batch size and write path.
- **Load testing:** Multiple devices, burst changes, reconnection scenarios; verify convergence and no data loss.
- **Real-time push:** Optional WebSocket (or long-poll) so devices get notified of changes without polling pull.
- **RAG sync:** Extend sync protocol to include RAG chunks (and optionally episodic) with same LWW/per-entity semantics.
- **Peer-to-peer:** Sync without a central server (e.g. local network discovery, direct device-to-device exchange).
- **Full CRDT for text:** If we add collaborative in-place editing of a single memory, consider RGA/Automerge for that field only.
