# P11 Phase 5 — Login & Authentication Design

**Purpose:** Design document for Phase 5 auth (user registration, login, guest experience, and migration). Share with the team for review. Use this file to start a new chat: *"Continue from @CAPSTONE/project_charters/P11_PHASE5_AUTH_DESIGN.md"* or *"Implement Phase 5 Auth from the design doc."*

**Related:** `P11_UNIFIED_REFERENCE.md` §8.7 (user_id FE ownership), §8.8 (Phase 5 items).
...
### 6.2 Guest Creation
- **FE Ownership**: The frontend generates the UUID using `crypto.randomUUID()` and stores it in `localStorage`. This avoids an unnecessary round-trip to the server and aligns with the frontend ownership goal (§8.7).
...
- `local_only` spaces never sync (consistent with Phase 4).
...
10. Sync engine: bind to authenticated `user_id` (Phase 4 alignment).
...
## 13. Phase 5 Implementation Plan & Task Checklist

### Backend Preparation
- [ ] **1. Setup User Database & Model:**
  - Initialize SQLite database configuration using SQLModel (`memory/db.py` or `config/database.py`).
  - Create the `User` DB Model (`memory/models/user.py`): UUID, email, password_hash, auth_type, migrated_guest_ids.
  - Set up Alembic (or basic SQLModel script) to initialize/migrate the schema.

- [ ] **2. Security & Context Middleware:**
  - Create `core/auth/security.py`: JWT utilities (encode/decode), password hash/verify (passlib).
  - Create FastAPI middleware `core/auth/middleware.py`: Read JWT or `X-User-Id` header, set into `contextvars`.
  - Refactor `memory/user_id.py.get_user_id()`: Read from `contextvars` first, fallback to header, then legacy file fallback.

- [ ] **3. Auth Endpoints:**
  - Implement `/auth/register` (`routers/auth.py`): Hash password, create DB user, return JWT. Hand off `guest_id` to Migration Service if provided.
  - Implement `/auth/login`: Verify password, return JWT. Hand off `guest_id` to Migration Service if provided.
  - Implement `/auth/me`: Decodes valid JWT, returns user info.

- [ ] **4. Guest -> Registered Migration Service:**
  - Create `memory/auth/migration.py` containing `migrate_guest_to_registered(source_guest_id, target_user_id)`.
  - Wrap inside an atomic transaction.
  - Update `user_id` in Qdrant payloads, Neo4j `User` edges, Neo4j `Fact` node properties, and sync logs.

- [ ] **5. Data Layer Scoping:**
  - Refactor all data layer callers (`QdrantStore`, Neo4j `knowledge_graph.py`, `routers/remme.py`, `sync/engine.py`) to invoke `get_user_id()` dynamically per-request rather than caching it at initialization.

### Frontend Preparation
- [ ] **6. Authentication State & Store:**
  - Update `platform-frontend/src/store/`: Create an auth slice with Guest vs Logged-In state.
  - Generate and persist `user_id` via `crypto.randomUUID()` in `localStorage` for guests on first load.

- [ ] **7. API Layer Wiring:**
  - Update API interceptor/fetch wrappers (`platform-frontend/src/lib/api.ts`) to attach `Authorization: Bearer <token>` (if logged in) or `X-User-Id: <guest_id>` (if guest) on every request.

- [ ] **8. Login/Register UI:**
  - Build Auth Modal/Page (Login, Register).
  - Add explicit UX confirmation upon registration/login: *"Merge this device’s data into your account?"* (if guest ID has active local memory) and hand `guest_id` to backend.
  - Prompt user to enable cloud sync for `local_only` spaces post-login.

### Refinements & Validations
- [ ] **9. Enforce Space Rules:**
  - Backend validation: Guest users (auth_type="guest") can only create spaces with `sync_policy=local_only`.
- [ ] **10. Testing & Integration:**
  - Verify multi-tenant separation: Data from Guest A doesn't pollute Guest B.
  - Test migration bounds: Log out, register -> confirm facts and vectors moved over correctly.

---

**Next steps:** 
Once reviewed and approved, drop a message like: *"Looks good! Let's start with Backend Preparation step 1."*
